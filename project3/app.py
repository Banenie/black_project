...
@dash_app.server.route('/api', methods=['GET'])
def api_weather():
    city = request.args.get('city')

    if city is None:
        return jsonify({'error': 'Некорректное обращение к API'})
    
    location_key = get_location_city_key(city)
    weather_data = get_5days_weather_data(location_key)

    check_data = [check_weather(*j.values()) for j in weather_data]
    ...
    return jsonify({'weather_data': weather_data, 'verdict': verdict})
